...
int main() {
    // Menu data já definido no enunciado
    Produto menu[] = {
    ...
    // Contar produtos com "Atum" e "Pão" acima da média
    int contador = 0;
    for (int i = 0; i < n; i++) {
        if (menu[i].preco > media) {
            int tem_atum = 0;
            int tem_pao = 0;
            
            // Verificar ingredientes
            for (int j = 0; j < 5; j++) {
                if (strcmp(menu[i].ingredientes[j], "Açúcar") == 0) {
                    tem_atum = 1;
                }
                if (strcmp(menu[i].ingredientes[j], "Leite condensado") == 0) {
                    tem_pao = 1;
                }
            }
            
            // Se tem ambos os ingredientes, incrementar o contador
            if (tem_atum && tem_pao) {
                contador++;
            }
        }
